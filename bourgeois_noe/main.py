...
class NeuralNetwork:
    ...
    def __init__(
        self,
        input_size: int = 784,  # input neurons quantity
        hidden_size: int = 784,  # hidden neurons quantity
        output_size: int = 10,  # output neurons quantity
    ...
    def get_details(
        ...
        details_list = [
            f"{newline_char}{key}{colon_char}{value}"
            for key, (value, include) in attributes.items()
    ...
    def train(
        ...
        batch_rate=0.0005,  # batch size
        show_training_progress=True,  # whether to show the training progress
        weights_save_path="weights",  # path to save the weights
    ...
    def plot_training_progress(self):
        ...
        (accuracy_line,) = ax2.plot(
            self.accuracy_history, label="Accuracy", color="green"
        )
    ...
    def plot_results(self, results):
        ...
        plt.savefig(
            ""
            # "doc/out/accuracy/"
            + f"accuracy_max_{accuracy_max:.2f}_min_{accuracy_min:.2f}_" + f"{details}"
